...
const ConnectivitySection = () => {
  const connectivityCards = [
    ...
    },
    {
      ...
      items: [
        { name: 'Google Map Link', distance: '', link: true }
      ],
      color: 'bg-red-50'
    },
    {
    ...
    },
    {
      ...
      items: [
        { name: 'XX km from Nearest Metro/Station', distance: '' }
      ],
      color: 'bg-indigo-50'
    }
  ];
      ...
      >
        {connectivityCards.map((card, index) => (
          <motion.div
            ...
            whileHover="hover"
            className={`rounded-2xl p-6 ${card.color} border border-gray-100 shadow-sm`}
          >
            ...
            <ul className="space-y-3 pl-2">
              {card.items.map((item, i) => (
                ...
                >
                  ...
                  <span className={item.link ? 'text-blue-600 underline' : ''}>
                    {item.name}
                  </span>
                  {item.distance && (
